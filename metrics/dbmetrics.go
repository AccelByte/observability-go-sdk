...
func (dbMetric *PostgreDBMetrics) ObservePostgreDBMetric(dbType string, db *sql.DB) {
	dbStats := db.Stats()
	dbMetric.MaxOpenConnections.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.MaxOpenConnections))
	dbMetric.OpenConnections.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.OpenConnections))
	dbMetric.InUse.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.InUse))
	dbMetric.Idle.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.Idle))
	dbMetric.WaitCount.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.WaitCount))
	dbMetric.WaitDuration.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.WaitDuration))
	dbMetric.MaxIdleClosed.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.MaxIdleClosed))
	dbMetric.MaxIdleTimeClosed.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.MaxIdleTimeClosed))
	dbMetric.MaxLifetimeClosed.With(map[string]string{dbMetricLabelInstance: dbType}).Observe(float64(dbStats.MaxLifetimeClosed))
}
